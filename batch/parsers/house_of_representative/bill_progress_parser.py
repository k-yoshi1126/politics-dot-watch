...
class BillProgressParser:
    ...
    def _parse_date(self, date_str: str) -> Optional[date]:
        ...
        try:
            # 日付文字列からスペースを完全に削除
            date_str = date_str.replace(' ', '').replace('\u3000', '')  # 全角スペースも削除
    ...
    def parse(self) -> Dict[str, Union[str, int, date, None]]:
        ...
        try:
            # 1つ目のテーブル
            first_table = self.soup.find('table', {'class': 'table', 'summary': 'この表は横2列、縦20行の表です。'})
            if not first_table:
                ...
                for i, row in enumerate(first_table.find_all('tr')):
                    ...
                    if len(cells) == 2:
                        ...
                        if not key_span or not value_span:
                            logger.error(f"テーブル1の{i+1}行目: キーまたは値の要素が見つかりません")
                            continue
                                ...
                                if j < len(values):
                                    try:
                                        ...
                                        if key in ['議案提出回次', '議案番号']:
                                            result[key] = self._convert_to_int(values[j])
                                        # 日付フィールドの変換
                                        elif any(date_key in key for date_key in ['年月日', '日付']):
                                            result[key] = self._parse_date(values[j])
                                        # その他のフィールド
                                        else:
                                            result[key] = self._convert_empty_to_none(values[j])
                                    except Exception as e:
                                        logger.error(f"フィールド変換エラー: キー={key}, 値={values[j]}, エラー={str(e)}")
                                else:
                                    logger.error(f"値が不足しています: キー={key}, 必要な値の数={j + 1}, 実際の値の数={len(values)}")

            # 2つ目のテーブル
            second_table = self.soup.find('table', {'class': 'table', 'summary': 'この表は横2列、縦2行の表です。'})
            if not second_table:
                ...
                for i, row in enumerate(second_table.find_all('tr')):
                    ...
                    if len(cells) == 2:
                        ...
                        value_span = cells[1].find('span', class_='txt03')
                        if not key_span or not value_span:
                            logger.error(f"テーブル2の{i+1}行目: キーまたは値の要素が見つかりません")
                            continue
                        ...
                        try:
                            ...
                            logger.error(f"フィールド変換エラー: キー={key}, 値={value}, エラー={str(e)}")
