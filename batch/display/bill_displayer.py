...
class BillDisplayer:
    ...
    def display_bills(self, bills: List[BillData]):
        ...
        self.console.print(
            f"\n📊 スクレイピング結果: {len(bills)}件の議案を取得しました",
            style="bold blue",
        )
    ...
    def display_summary(self, bills: List[BillData]):
        ...
        summary_table.add_row(
            "📅 提出日情報あり", str(len([b for b in bills if b.submitted_date]))
        )
        summary_table.add_row(
            "📝 全文情報あり", str(len([b for b in bills if b.full_text]))
        )
        summary_table.add_row(
            "👤 提出者情報あり", str(len([b for b in bills if b.submitter]))
        )
    ...
    def display_bill_detail(self, bill: BillData):
        ...
        self.console.print(
            f"[bold]提出日:[/bold] {bill.submitted_date.strftime('%Y/%m/%d') if bill.submitted_date else '不明'}"
        )
